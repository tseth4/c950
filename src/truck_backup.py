...
class Truck:
    ...
    def remaining_capacity(self):
        ...
        return self.capacity - len(self.packages)
    ...
    def deliver_package(self, adjacency_matrix, speed_mph, route_index, current_time):
        ...
        print("truck id: ", self.id, " distance from: ",
              prev_index, " ", current_index, " ", distance)
        self.total_distance += distance
    ...
    def process_deliveries(self, adjacency_matrix, cutoff_time=None):
        ...
        while undelivered_packages:
            ...
            print("current trip values: ", [
                  pkg.address_index for bucket in self.current_trip.values() for pkg in bucket])
            # Optimize the route for the current trip
            self.optimize_route(adjacency_matrix)
            print("route: ", self.route)

            print(
                f"Truck-{self.id} is starting a trip with {self.current_trip.count} packages.")
            ...
            for i in range(1, len(self.route)):  # Skip hub (index 0)
                ...
                current_time, current_index = self.deliver_package(
                    adjacency_matrix, speed_mph, i, current_time)
                ...
                if packages_at_address:
                    for package in packages_at_address:
                        print("truck id: ", self.id, " delivering package ",
                              package.id, " at address_id ", package.address_index)
